...
- `AgentExecutor.execute_agent_node()` is called by FlowRunner with a `parent_execution` — records agent details under the flow's tree
...
**When modifying flow execution**: any change to `FlowRunner`, `tool_runner`, or `AgentExecutor` automatically applies to both local and production — the local flow child and the hosted flow-runner container run the same `src.tasks.run_flow` entrypoint. No branching on environment inside the executors.
