...
- **Endpoint**: `POST /flows/{flow_id}/export` (body `{user_id, agent_llms}`) → `application/zip` with `Content-Disposition`. `agent_llms` maps agent node ids to LLM config names, same contract as `/execute` (no fallback; missing selection → 422 `FlowExportError`). Frontend: "Export flow" button in the flow-controls toolbar → `exportFlow()` in `api.ts` (blob download).
