...
- **Three executor types** following a factory pattern:
  - `src/runners/tool_runner.py` — in-process tool compilation (replaced the pickle-subprocess `tool_executor.py`): `compile_tool(tool)` execs the tool's full `script_code` into fresh globals and returns its `main_function` callable — native Python objects flow between nodes with zero serialization. Also home of the type helpers `parse_imports_and_classes` / `eval_type_string` (used by `pydanticai_tool_converter.py`). Only flow-running processes import it — **user tool code never runs inside the backend API process** (only the local flow child, the hosted flow-runner container, or pytest)
  - `src/runners/flow_runner.py` — `FlowRunner`, the codegen flow engine (replaced the old `flow_executor.py`): generates orchestrator source from graph_config via `src/factories/flow_to_script_factory.py`, `exec`s it, and runs the async `orchestrate()`. Chains at the same topological level run concurrently. Tool nodes run under a global timeout (`TOOL_TIMEOUT_SECONDS` env var, default 300s — soft: the worker thread survives, its late result is discarded). Also owns in-memory resume: on failure the runner retains `ctx.state` (raw node outputs) so a resume re-runs only failed/edited nodes and their downstream (graph-aware fingerprint invalidation); locally the failed run stays resident in its per-run child process (handle held in `src/runners/live_run_store.py`, single slot), in HOSTED mode it stays resident in its flow-runner container awaiting a redis `resume:<execution_id>` message
  - `src/runners/agent_runner.py` — prepare-time agent compilation (mirrors `tool_runner.compile_tool`): `compile_agent(sub_node_config, tool_records, provider_config)` returns a `BuiltAgent` (PydanticAI `Agent` + `class_to_path` output-path routing map) with credentials bound into the model object via `AnthropicProvider`/`OpenAIProvider` — no env-var side effects, no langfuse, no DB writes, no `.run()`. `FlowRunner._prepare_agents` compiles every agent node's sub-agents once per run into `ctx.agents_by_node` and captures each agent's graph_config in `ctx.agent_graphs`; `resume()` recompiles both on the fresh session (edited agents take effect on resume). Errors (missing agent, no LLM selected, unknown provider) surface at prepare time, before any node runs
  - `src/executors/agent_executor.py` — unified BFS graph executor for all agents (single-node and multi-agent), cyclic graphs supported; execution only — runs pre-built `BuiltAgent`s handed down from `ctx.agents_by_node` (`execute_agent_node(graph_config, input_text, built_agents, parent_execution)`), does zero DB reads at run time, and reflection loops reuse the same `BuiltAgent` across iterations
...
```
The schema allows standalone agent executions (`parent_id=NULL, type='agent'`), but no code path creates them today — agents run only as flow nodes. The `messages` table no longer exists.
...
**Execution tree** — Both `FlowRunner` and `AgentExecutor` record executions to the self-referencing `executions` table:
- `FlowRunner.run(initial_input, conda_env, execution_id=None)` creates a top-level `Execution(type='flow')` and child records for each node (trigger, tool, agent). The optional `execution_id` parameter reuses a pre-existing row (used by the Celery task path — see "Production Execution" below) instead of creating a new one.
- **Node I/O is stored as bounded previews only**: per-node `input_data`/`output_data` are ≤1000-char strings (`_preview` — `df.head()` for DataFrames, duck-typed). Raw outputs live only in the runner's in-memory `ctx.state`. The root row keeps the full `initial_input`. Agent multi-node sub-rows keep their `[:2000]` dict pattern in `agent_executor.py`. `final_output` in execute/resume responses is also a preview.
- **Resume**: `POST /executions/{execution_id}/resume` (body `{user_id}`) resumes a failed flow run from its in-memory checkpoint, reusing the ORIGINAL root execution row (child rows are updated in place — the `flowrunner` DB role has no DELETE grant). Resume is graph-aware: it re-fetches the flow, regenerates the orchestrator, and invalidates cached outputs for any node whose config/edges/tool/agent changed (fingerprint diff on `updated_at` + config JSON) plus all downstream nodes; unchanged upstream results short-circuit. 404 = no such execution, 409 = not failed, 410 = checkpoint gone (server restart, superseded by a newer failure, or resume-window expiry). LOCAL keeps the failed run's child process resident and registered in the single-slot `LiveRunStore`; a new failure supersedes (and shuts down) the previous child and success reaps it. The endpoint signals the child over stdin and polls the DB for the outcome, using the failure's `completed_at` as a transition witness.
- **In-run tool testing**: `POST /executions/{execution_id}/test` (body `{node_id}`) tests one tool node inside a resident failed run against its real `ctx.state` inputs. The resident process re-fetches the tool (picking up edits), compiles it, runs it under `TOOL_TIMEOUT_SECONDS` with stdout/stderr captured, and merges the previewed outcome into the root row's `execution_metadata["last_test_result"]` — **transient: no execution rows are written and `ctx.state` is untouched**. The endpoint signals the resident process (stdin locally / redis `test:<execution_id>` hosted, receivers 0 → 410) and polls that metadata field for its `request_id`. 409 if the run isn't `failed`. Nodes downstream of the failure return an "inputs unavailable" error result.
- `AgentExecutor.execute_agent_node()` is called by FlowRunner with a `parent_execution` — records agent details under the flow's tree. Agents execute only inside flows; there is no standalone agent-execution endpoint or method
...
- The LangFuse client is initialized in `src/observability/langfuse_tracing.py` (`langfuse_client`, `langfuse_observe`, `LANGFUSE_AVAILABLE`) — imported by the backend's trace/score proxy endpoints and both executors. `Agent.instrument_all()` is called by `agent_executor.py` and `evaluation_executor.py` only (execution-side: the modules that run agents); the backend no longer imports `agent_executor` or instruments PydanticAI
...
- While the run is failed, tool-node rows show a **Test** button (`testTool` in `api.ts` → `POST /executions/{id}/test`): runs that node inside the resident process against real upstream data and renders the transient result (status/result/stdout/stderr/error) in the node's details — no execution rows are recorded
...
- `POST /executions/{id}/test` — in-run tool test against a resident failed run (transient; see "Execution Recording" above)
...
- **`ENVIRONMENT=local`** (dev default): `POST /flows/{id}/execute` pre-creates an `Execution(status='pending')` row and spawns a **per-run child process** (`src/runners/local_flow_child.py` → `python -m src.tasks.run_flow`, the same entrypoint the hosted container uses) with `FLOW_RUNNER_RESUME_TRANSPORT=stdin` and `FLOW_RUNNER_INSTALL_PACKAGES=false`. The endpoint then blocks its threadpool thread polling the DB (0.3s, fresh session per poll) and returns the same synchronous response shape as before — no Redis, no Celery, no Podman required. The child runs under the flow's environment: `Flow.conda_env` unset → the backend's interpreter; a directory → `<dir>/bin/python` (conda env or venv, `<dir>/bin` prepended to PATH); a file → that interpreter. **The env must already contain the runner deps** (`deploy/flow-runner/requirements.txt` minus psycopg2/redis) — nothing is auto-installed; agents also run inside the child, so agent flows need the provider SDKs there too. On failure the child stays resident (parked on a stdin selector, near-zero CPU) and is registered in `LiveRunStore` for resume/test; stdin EOF (backend death) makes an idle child exit. Safety nets: child exits without a terminal status → marked failed after a 2s grace; run exceeds `FLOW_RUNNER_TIMEOUT_SECONDS` (default 3600) → killed and marked failed.
...
**Celery task** (`src/tasks/tasks.py::execute_flow_task`) — receives `(flow_id, user_id, initial_input, conda_env, execution_id, agent_llms)`. Default behavior: spawns an `llmhub-flow-runner` Podman container via `podman-py` bound to `CONTAINER_HOST` (the host's rootless socket), then **watches the execution row (~5s DB poll) until it reaches a terminal status and detaches** — it does NOT wait for container exit, because a failed run's container stays resident for the resume window and blocking would pin a worker slot (worker runs `--concurrency=2`). Logs are relayed by a daemon thread until detach. Falls back to in-worker execution when `FLOW_RUNNER_USE_PODMAN=false` (useful for testing the Celery layer without Podman; resume is unsupported on this path — the runner is GC'd when the task ends).
...
- Reads `FLOW_RUNNER_FLOW_ID`, `FLOW_RUNNER_USER_ID`, `FLOW_RUNNER_EXECUTION_ID`, `FLOW_RUNNER_INITIAL_INPUT` (JSON), `FLOW_RUNNER_CONDA_ENV`, plus resume plumbing `FLOW_RUNNER_REDIS_URL` and `FLOW_RUNNER_RESUME_TIMEOUT_SECONDS`, from env vars set by the worker's `podman run -e`
- Opens its own DB session via `DatabaseManager().get_session()`, runs `install_required_packages_for_flow(session, flow_id)` to `uv pip install --user` every package listed in `required_packages` across the flow's tools and agent-attached tools (best-effort — warns but continues on failure), then calls `FlowRunner.run(..., execution_id=...)` — **exactly the same code path as local mode**. Tools still run as host-style subprocesses; they don't know they're in a container.
- **On failure the container stays resident** (`_await_resume_redis` in `run_flow.py`; `_await_resume` dispatches on `FLOW_RUNNER_RESUME_TRANSPORT` — `stdin` for the local child, otherwise redis): it subscribes to redis `resume:<execution_id>` and `test:<execution_id>` and waits up to `FLOW_RUNNER_RESUME_TIMEOUT_SECONDS` (default 1800; window re-arms after each failed resume attempt; test messages don't consume it). On a resume message it re-runs `install_required_packages_for_flow` (a tool edit may add packages; gated on `FLOW_RUNNER_INSTALL_PACKAGES != "false"`) then `runner.resume()`; on a test message it runs `_handle_test_request` (in-run tool testing). Exits 0 on eventual success, 1 on window expiry. The API's resume endpoint publishes to that channel; `PUBLISH` receiver count 0 → HTTP 410.
...
**Env var forwarding** (`_FORWARDED_ENV_VARS` + resume vars in `tasks.py`): worker → flow-runner forwards `DATABASE_URL`, `SQL_DEBUG`, `ENVIRONMENT`, `LANGFUSE_*`, `FLOW_RUNNER_REDIS_URL` (from `CELERY_BROKER_URL`), `FLOW_RUNNER_RESUME_TIMEOUT_SECONDS`. LLM provider credentials are NOT forwarded — in production they live in Postgres (per-user), looked up at runtime via `DATABASE_URL`. Local dev still uses `~/.llm_hub/config.yaml`.

**Safety net**: if the flow-runner container never reaches a terminal status within `FLOW_RUNNER_TIMEOUT_SECONDS` (crashed or hung before writing to the DB), the worker's watch loop calls `_mark_failed_if_still_running(execution_id, reason)` and kills the container so the frontend doesn't see it stuck.

**When modifying flow execution**: any change to `FlowRunner`, `tool_runner`, `agent_runner`, or `AgentExecutor` automatically applies to both local and production — the local flow child and the hosted flow-runner container run the same `src.tasks.run_flow` entrypoint. No branching on environment inside the executors.

**Standalone tool testing** (running a tool outside any flow run, e.g. `POST /tools/{id}/run`) is a documented follow-up — not implemented yet. Today's in-run testing only works against a resident failed run.

### Flow Export ("Eject")

`src/exporters/flow_exporter.py` deconstructs a flow into a standalone, human-readable Python module (zip download): `tools/<slug>.py` (verbatim `script_code`), `agents/<slug>.py` (the `compile_agent` recipe emitted as source — provider/model baked in, API keys read from `os.environ`, never embedded), a `flow.py` orchestrator with each node's input statically inlined as explicit kwargs (mirrors `flow_runner._build_node_input` branch by branch; small `_as_text`/`_pick`/`_merge_auto` helpers are emitted only when a case can't be resolved statically), `requirements.txt` (union of tool `required_packages` minus infra deps, plus pydantic-ai/SDK pins from `deploy/flow-runner/requirements.txt` iff agents), `README.md`, and `__main__.py`. Exported code has zero llmhub imports and runs on `uv pip install -r requirements.txt`.

- **Additive only** — pure text generation (no exec, no `pydantic_ai` import, safe in the backend API process); the runtime hot path is untouched. Topology reuses `decompose_into_chains`/`compute_chain_levels`.
- **Endpoint**: `POST /flows/{flow_id}/export` (body `{user_id, agent_llms, parallel}`) → `application/zip` with `Content-Disposition`. `agent_llms` maps agent node ids to LLM config names, same contract as `/execute` (no fallback; missing selection → 422 `FlowExportError`). `parallel: false` emits a fully sequential flow.py (no gather/to_thread). Frontend: "Export flow" button in the flow-controls toolbar opens an options dialog (Parallel toggle) → `exportFlow()` in `api.ts` (blob download).
- **Phase 1 scope**: tool-only flows + single-node agents. Multi-node/looping agents → 422. Exported tool calls: direct for sequential nodes, `asyncio.to_thread` inside `asyncio.gather` levels; no per-tool timeout (platform policy, noted in the export's README).
- Tests: `tests/test_flow_exporter.py` (all emitted files must compile; wiring goldens; credential-leak and no-`src.`-import guards).

### Key Patterns
- **graph_config** is the central data structure: a JSON object with `nodes` (dict), `edges` (list of from/to mappings), `entry_point`, and `exit_points`. The frontend builds it from the canvas; the backend traverses it for execution. Edge `mapping` shapes: absent = passthrough, `{"": param}` = whole output → param, `{field: param}` = field → param, `{field: ""}` = field → whole input (an expanded dict output feeding a generic-input target — agent nodes or schema-less tools; respected by `flow_runner._build_node_input` and the flow exporter).
- **Streaming** uses SSE (Server-Sent Events) via FastAPI `StreamingResponse`. The frontend reads streams with `ReadableStream` reader pattern. Used for AI code generation and prompt generation (not agent execution — flows report progress via the executions tree).
...
- `TOOL_TIMEOUT_SECONDS` (default 300): global per-tool-node execution timeout, applied in `flow_runner._run_tool_node` and to in-run tests (replaces the old hard-coded 300s subprocess timeout). No per-tool column.
- Flow-child env vars (set by the spawner, not `.env`): `FLOW_RUNNER_RESUME_TRANSPORT` (`stdin` for local children; unset → redis in hosted containers), `FLOW_RUNNER_INSTALL_PACKAGES` (`false` for local children — flow envs are user-managed; hosted default installs `required_packages`), plus the `FLOW_RUNNER_{FLOW_ID,USER_ID,EXECUTION_ID,INITIAL_INPUT,CONDA_ENV,AGENT_LLMS}` contract shared with the hosted container.
- Production-only env vars (set in `deploy/podman-compose.yml`, not `.env`): `CELERY_BROKER_URL`, `CELERY_RESULT_BACKEND`, `CONTAINER_HOST`, `FLOW_RUNNER_IMAGE`, `FLOW_RUNNER_NETWORK`, `FLOW_RUNNER_USE_PODMAN`, optional `FLOW_RUNNER_MEMORY`, `FLOW_RUNNER_CPUS`, `FLOW_RUNNER_TIMEOUT_SECONDS` (also honored by the local execute facade).
